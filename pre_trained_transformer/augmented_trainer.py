"""This code was used to fine-tune the pre-trained ViT on an augmented dataset."""
...
dataset = load_dataset("imagefolder", data_dir=DATASET_DIRECTORY) # load dataset
...
_transforms = Compose([v2.RandomGrayscale(p=0.03), # composition of transforms
                       v2.RandomHorizontalFlip(p=0.5), 
                       v2.RandomVerticalFlip(p=0.5),
                       RandomApply(ModuleList([v2.GaussianBlur(kernel_size=(5, 9), sigma=(0.1, 5.))]), p=0.03),
                       RandomApply(ModuleList([v2.ColorJitter()]), p=0.1),
                       RandomApply(ModuleList([v2.RandomPerspective()]), p=0.08)]
